...
const Benefit = ({ icon, title, index }: { icon: string, title: string, index: number }) => (
    ...
    >
        <div className="w-20 h-20 bg-white shadow-xl rounded-2xl flex items-center justify-center group-hover:bg-amethyst-primary transition-all duration-500 transform group-hover:-translate-y-2 border border-amethyst-lightest">
        ...
        <span className="text-[10px] uppercase tracking-[0.2em] font-black text-amethyst-dark group-hover:text-amethyst-primary transition-colors">{title}</span>
    </motion.div>
);
        ...
        <section className="relative z-20 -mt-16 pb-20">
            <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col items-center">
                ...
                <div className="text-center w-full max-w-4xl">
                    <h3 className="text-3xl font-serif text-amethyst-dark mb-10">Showcasing home designs based on <span className="text-amethyst-primary italic font-bold">your home size</span></h3>
                    <div className="flex flex-wrap justify-center gap-4 bg-white/50 p-2 rounded-2xl border border-amethyst-light/20 w-fit mx-auto shadow-lg">
                        {propertyTypes.map((type) => (
                            <button
                                key={type}
                                onClick={() => setActiveTab(type)}
                                className={`px-10 py-4 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${activeTab === type
                                        ? "bg-amethyst-primary text-white shadow-xl shadow-amethyst-primary/30"
                                        : "text-amethyst-dark hover:bg-amethyst-lightest"
                                    }`}
                            >
                                {type}
                            </button>
